...
def execute_trade(side, price, execution_reason="Traditional signal"):
    ...
    try:
        ...
        order_data = MarketOrderRequest(
            symbol="NVDA",  # Keep as NVDA (this line is already correct)
            qty=quantity,
            side=side,
...
"""
    ...
    logger.info(f"📊 SYMBOL         : {TICKER} (NVDA Stock)")  
...
def print_signal_update(latest_bar, current_price, current_position, signal_count=1):
    ...
    logger.info(f"Position       : {current_position} NVDA")
...
def print_waiting_message(next_check_time):
    ...
    logger.info(f"{Fore.GREEN}Market Status  : Stock Market Hours (9:30 AM - 4:00 PM ET){Style.RESET_ALL}")
                ...
                if last_check_minute != current_minute:
                    ...
                    if latest_bar['Buy']:
                        if current_position > 0:
                            logger.info(f"Already holding {current_position} NVDA, no action taken.")
                        else:
                            # Validate with AI
                    ...
                    # Process sell signal with AI validation
                        ...
                        if current_position > 0:
                            ...
                            if should_execute:
                                logger.info(f"🤖 AI VALIDATION PASSED: Selling {current_position} NVDA at ${current_price:.4f}")
    ...
    if trade_history:
        ...
        if current_position > 0:
            ...
            try:
                ...
                logger.info(f"Open position  : {current_position} NVDA valued at ${position_value:.2f}")
            except Exception as e:
