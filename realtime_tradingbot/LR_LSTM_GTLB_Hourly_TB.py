...
# Ensemble Models Configuration for GTLB
ENSEMBLE_CONFIG = {
    'linear_regression': {
        'display_name': 'Linear Regression',
        'model_file': 'linear_regression_GTLB_1h_model.pkl',
        'config_file': 'linear_regression_GTLB_1h_config.pkl',
        'type': 'traditional_ml',
        'weight': 0.5  # Equal weight for ensemble
    },
    'lstm': {
        'display_name': 'LSTM',
        'model_file': 'lstm_GTLB_1h_model.pt',
...
def load_ensemble_models():
    """Load both Linear Regression and LSTM models for ensemble prediction"""
    models = {}
...
def fetch_and_prepare_data_for_ai(ticker_symbol, days_back=10):
    ...
    try:
        ...
        # Fetch GTLB data using yfinance
...
def generate_ensemble_prediction(current_price, historical_data):
    """Generate ensemble prediction using both Linear Regression and LSTM models"""
    try:
        ...
        if 'linear_regression' in models:
            try:
                ...
                if target_ts is not None and len(target_ts) >= lags:
                    ...
                    logger.warning("⚠️ Insufficient data for Linear Regression prediction")
        ...
        if 'lstm' in models:
            try:
                ...
                if target_ts is not None and len(target_ts) >= input_chunk_length:
                    ...
                    logger.warning("⚠️ Insufficient data for LSTM prediction")
...
{'=' * 70}
{Fore.CYAN}
 🤖 ENSEMBLE AI-ENHANCED ZERO LAG MACD TRADING BOT 🤖
...
"""
    ...
    logger.info(f"🤖 STRATEGY       : Ensemble AI-Enhanced Zero Lag MACD Trading Bot")
    ...
    logger.info(f"🤖 AI MODELS      : Linear Regression + LSTM Ensemble")
...
def print_ai_validation_stats():
    """Print ensemble validation statistics"""
    stats = ai_validation_stats
    print(f"{'=' * 60}")
    logger.info(f"🤖 ENSEMBLE VALIDATION STATISTICS")
...
def print_waiting_message(next_check_time):
    ...
    print(f"{'=' * 60}")
    logger.info(f"{Fore.BLUE}🤖 ENSEMBLE BOT STATUS : MONITORING {TICKER}{Style.RESET_ALL}")
    ...
    logger.info(f"🤖 AI Models     : Linear Regression + LSTM ({'ACTIVE' if ENABLE_AI_VALIDATION else 'INACTIVE'})")
    print(f"{'=' * 60}")
