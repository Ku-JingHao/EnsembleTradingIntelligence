...
TICKER = "NVDA"  
YFINANCE_TICKER = "NVDA"  # Yahoo Finance uses different format for crypto
INTERVAL = "1m" 
...
def prepare_deep_learning_data(data, interval="1m"):
    ...
    try:
        ...
        # Create a continuous minute-by-minute index
        start_time = clean_index.min()
        end_time = clean_index.max()
        continuous_index = pd.date_range(start=start_time, end=end_time, freq='T')
...
def generate_ai_prediction(current_price, historical_data):
    ...
    try:
        ...
        target_ts, scaler = prepare_deep_learning_data(historical_data, interval="1m")
...
def print_signal_update(latest_bar, current_price, current_position, signal_count=1):
    ...
    logger.info(f"Price          : ${current_price:.2f}")  # More decimals for crypto
