...
TICKER = "NVDA" 
YFINANCE_TICKER = "NVDA"  # Yahoo Finance format (same for stocks)
INTERVAL = "1h"  # Keep as 1h for hour bot
...
def generate_ml_prediction(current_price, historical_data):
    ...
    try:
        ...
        target_ts, covariate_ts, scaler = prepare_traditional_ml_data(historical_data, interval="1h")
...
def print_waiting_message(next_check_time):
    ...
    logger.info(f"{Fore.GREEN}Market Status  : Stock Market Hours (9:30 AM - 4:00 PM ET){Style.RESET_ALL}")  # Change from 24/7 crypto
    ...
    try:
        while True:
            try:
                ...
                if last_check_hour != current_hour:
                    ...
                    # Process sell signal with AI validation
                        ...
                        if current_position > 0:
                            ...
                            if should_execute:
                                logger.info(f"🤖 AI VALIDATION PASSED: Selling {current_position} {TICKER} at ${current_price:.4f}")
                ...
                time.sleep(60)  # Check every second (more frequent for 1-min strategy)
                
            except Exception as e:
                logger.error(f"Error in live trading execution: {e}")
                logger.error(f"{'=' * 60}")
                time.sleep(60)  # Shorter retry time for 1-min strategy
